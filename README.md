...
Click the green **Generate Scripts** button to save `01-deploy.sh` and `02-postdeploy.sh` to the `scripts/` folder, then run them:

![deploy](docs/images/helper-deploy.jpg)

```bash
./scripts/01-deploy.sh
./scripts/02-postdeploy.sh
```
